...
train_dat = readtable(strjoin(["../../Input_Data/Train_Val_Data/X_train_temp_",site,".csv"],''));
train_dat = [train_dat.Rn,train_dat.USTAR,train_dat.SRad];
train_dat = array2table(train_dat, 'VariableNames',{'NETRAD','USTAR','SW_IN_F_MDS'});
row_index = find(ismember(data.NETRAD,train_dat.NETRAD,'rows') & ismember(data.USTAR,train_dat.USTAR,'rows') & ismember(data.SW_IN_F_MDS,train_dat.SW_IN_F_MDS,'rows'));
data = data(row_index,:);
%% 
DateTime = data.DateTime; % DateTime "%Y-%m-%d %H:%M:%H"
DateTime = datetime(DateTime);
Year	      = year(DateTime); %Year
Month	      = month(DateTime); %Month
Day	          = day(DateTime); %Day
Hour	      = hour(DateTime) + minute(DateTime)./60; %Hour
Hour1 = floor(Hour);
Minute1 = (Hour - floor(Hour))*60;
DateTime=datetime(Year,Month,Day,Hour1,Minute1,0,'Format','yyyy-MMM-dd hh:mm:ss');
%-----------------   Load Energy data     -----------------% 
...
Min=[2.5 0 0]'; % 'Km', 'm','gc_ref','alpha','beta'
Max=[4.5 1 1]';
par=[3.5 0.3 0.3]';
Km = par(1);
z0s = 0.01;
...
Kml=mle(Parameters_keep(1,upgrade/3:(upgrade-1)),'distribution','gev');
Km=Kml(3);
ml=mle(Parameters_keep(2,upgrade/3:(upgrade-1)),'distribution','gev');
m=ml(3);
gc_refl=mle(Parameters_keep(3,upgrade/3:(upgrade-1)),'distribution','gev');
gc_ref=gc_refl(3);
%calculate maximum likelihood fluxes
[Es,Ei,Tr,rsc,rss]=ETestimation_emp(Zh,Ta_Avg,e_Avg,...
    LAI,...
    Prss_Avg,ustar,U...
    ,As,A,theta,theta_2,VPD_Avg,Km,z0s,cd,dl,m,gc_ref,alpha,beta,theta_w,theta_s);
...
r = corrcoef(modf,obsv, 'rows','complete');
r = r(2);
rmse = sqrt(nanmean((obsv - modf).^2));
%create plots
save('Parameters/MCMC_out_calib_Km_m_gcref.mat')